...
Apply this skill whenever you need to find information in Slack — locating messages, conversations, files, or people, or gathering context before answering a question about Slack activity.
...
- **Natural language** (e.g., "What is the deadline for project X?") — best for fuzzy, conceptual searches.
- **Keywords** (e.g., `project X deadline`) — best for finding specific, exact content.
...
- **Private channel access.** Use `slack_search_public_and_private` when you need to include private channels — requires user consent.
