...
- [ ] `sign-ins` CLI command accepts `--app`, `--error-code`, `--country`, `--risk-level` filter flags
- [ ] `risky-users` CLI command accepts `--risk-state` filter flag
- [ ] `azure_ad_ual_inbox_rules` docstring shows the correct default (6h not 72h)
