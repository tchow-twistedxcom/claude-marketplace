...
## Acceptance Criteria
- [ ] `auth.py` AuthError does not include `error_description` in message
- [ ] `server.py` UAL blob `except Exception: pass` replaced with typed exception handlers that log to stderr
- [ ] `server.py` `_triage_one` UAL fetch exception includes `as e` and logs warning to stderr
